...
var WFC = function({nd,weights,rules,wave}){
  ...
  var wavefront = {};
  function coord(k){ return k.split(",").map(x=>parseInt(x)); }
  function entropy(x){
    var one = 0; for (var i=0;i<x.length;i++){ one += x[i]*weights[i]; }
    var S=0; for (var i=0;i<x.length;i++){ var pi = x[i]*weights[i]/one; if (pi!=0){ S -= pi*Math.log(pi);} }
    return S;
  }
  function collapse(x){
    var one=0; for (var i=0;i<x.length;i++){ one += x[i]*weights[i]; }
    var r=Math.random()*one; for (var i=0;i<x.length;i++){ r -= x[i]*weights[i]; if (r<0){ var y=new Array(x.length).fill(0); y[i]=1; return y; }}
  }
  function neighborable(d,a,b){
    var didx=d.indexOf(1); if (didx<0){ didx=d.indexOf(-1); [a,b]=[b,a]; }
    for (var i=0;i<rules.length;i++){ if (didx==rules[i][0]||"yxz"[didx]==rules[i][0]){ if (a==rules[i][1] && b==rules[i][2]){ return true; }}}
    return false;
  }
  function propagate(p){
    var stack=[p];
    while(stack.length){ p=stack.pop(); var dirs=[]; for (var i=0;i<nd;i++){ var d0=new Array(nd).fill(0); d0[i]=-1; dirs.push(d0); var d1=new Array(nd).fill(0); d1[i]=1; dirs.push(d1);} 
      for (var i=0;i<dirs.length;i++){
        var q=[]; for (var j=0;j<p.length;j++){ q.push(p[j]+dirs[i][j]); }
        var x=wavefront[p]; if (x==undefined){ x=wave[p]; }
        var y=wavefront[q]; if (x==undefined){ x=wave[q]; }
        if (typeof y=='number'|| typeof y=='undefined'){ continue; }
        else if (typeof x=='number' && typeof y=='object'){
          var mod=false; for (var j=0;j<y.length;j++){ if (y[j]==0){ continue } if (y[j]>0 && !neighborable(dirs[i],x,j)){ y[j]=0; mod=true; }} if (mod){ stack.push(q);} }
        else if (typeof x=='object' && typeof y=='object'){
          var mod=false; for (var j=0;j<y.length;j++){ if (y[j]==0){ continue } var ok=false; for (var k=0;k<x.length;k++){ if (x[k]>0 && y[j]>0 && neighborable(dirs[i],k,j)){ ok=true; break; }} if (!ok){ y[j]=0; mod=true; }} if (mod){ stack.push(q);} }
        else { throw Error("Invalid propagation parameter",x,y); }
      }
    }
  }
