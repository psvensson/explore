...
describe('stair rule openness', () => {
  ...
  test('each stair prototype has open forward and backward neighbor possibilities filtered by face openness heuristic', () => {
    ...
    expect(stairs.length).toBe(2); // 2 base stairs with transforms in metadata
    const nonStairs = tilePrototypes.filter(p=> !(p.meta && p.meta.role==='stair'));

    for (const stair of stairs){
      const { axis, dir } = stair.meta;
      console.log(`\nChecking stair: axis=${axis}, dir=${dir}, tileId=${stair.tileId}`);
      if (axis==='z'){
        ...
        // Debug: check what the actual voxel value is
        const v = stair.voxels;
        if (forwardFace === 'posZ') {
          console.log(`  Checking posZ face: v[2][1][1] = ${v[2][1][1]}`);
        } else {
          console.log(`  Checking negZ face: v[0][1][1] = ${v[0][1][1]}`);
        }
